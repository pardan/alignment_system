...
class AutoAlignmentScheduler:
    ...
    def begin_local_scan(self, reason="signal_loss"):
        """Start one automatic local recovery scan and count it once."""
        if self.state == self.ALIGNING:
            return False
        if reason != "manual_button" and not self._automatic_attempt_allowed():
            return False
        return self._start_scan(reason)
